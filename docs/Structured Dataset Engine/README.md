...
- Routing: `KnowledgeSDK::IntentClassifier` detects `health`, `finance`, `crm`, `trading`, `knowledge`, or `generic`, then applies fixed Dataset → search → analysis → planning → proposal → specialized graph → Capture → clarification precedence. Named medication Intents cover create, replace, pause, resume, stop, dose evolution, and recurrence evolution; measurement and finance Intents remain unchanged. Weak template keywords do not claim read/analysis questions, and neither graph nor Capture is a fallback. Dataset proposals use the existing proposal, approval, submission, Engine, Event Bus, and Activity components.
